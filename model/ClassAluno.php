<?php
	  ...
	  function CadastrarAluno($nome,$nascimento,$cep,$logradouro,$numero,$complemento,$bairro,$cidade,$uf,$inscricao,$cpf,$rg,$email,$celular){
		       ...
		       $funcao->msg('error','E-mail é Inválido');
                  ...
                  else if($logradouro=="" || $bairro=="" || $cidade=="" || $uf=="")
                  {
	               ...
	               $sql = "INSERT INTO aluno( nome,data_nasc,cep,logradouro,numero,complemento,bairro,cidade,uf,data_inscr,cpf,rg,email,celular) VALUES ('".$nome."','".$nascimento."','".$cep."','".$logradouro."','".$numero."','".$complemento."','".$bairro."','".$cidade."','".$uf."','".$inscricao."','".$cpf."','".$rg."','".$email."','".$celular."')";
			 
			 $result = $this->conexao->query($sql);
			 
			 if(!$result){
			  if($this->conexao->errno==1062) $funcao->msg('info','CPF já possui cadastro');
                          if($this->conexao->errno!=1062) echo $this->conexao->error;
